# Slightly adapted from https://github.com/Olivier-true/MicroPython-Memory-Friendly-HTTP-Requests
import socket, ssl
...
def post(url, data=b"", headers={}, recvsize=1024):
    ...
    sock = socket.socket()
    sock.connect((hn, p))
    sock = ssl.wrap_socket(sock, server_hostname=hn) if pr == 'https' else sock
    headers1 = [
...
def get(url, headers={}, recvsize=1024):
    pr, hn, p, ph = parse_url(url)
    sock = socket.socket()
    addr = socket.getaddrinfo(hn, 443)[0][-1]
    sock.connect(addr)
    sock = ssl.wrap_socket(sock, server_hostname=hn) if pr == "https" else sock
